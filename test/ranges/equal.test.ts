...
describe.skip('SemVer \'Equal\' Range (=): tests', function () {
  ...
  context(' = /', function () {
    ...
    it('',
      async function () {
        packageJson.dependencies[packageName] = '8.0.1';
        const result = await resolver.resolve(packageJson);
        expect(result.dependencies[packageName]).to.be.equal('8.0.1');
      });

    it('',
      async function () {
        packageJson.dependencies[packageName] = '8.2.x';
        const result = await resolver.resolve(packageJson);
        expect(result.dependencies[packageName]).to.be.equal('~8.2.2');
      });
    ...
    it('',
      async function () {
        packageJson.dependencies[packageName] = '=8.0.x';
        const result = await resolver.resolve(packageJson);
