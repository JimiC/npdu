...
describe.skip('SemVer \'Less Than Or Equal To\' Range (<=): tests', function () {
  ...
  context(' <= /', function () {
    ...
    it('',
      async function () {
        packageJson.dependencies[packageName] = '<=8.1';
        const result = await resolver.resolve(packageJson);
        expect(result.dependencies[packageName]).to.be.equal('>=8.0.3 <8.2.0');
      });
    ...
    it('',
      async function () {
        ...
        const result = await resolver.resolve(packageJson);
        expect(result.dependencies[packageName]).to.be.equal('^8.2.2');
      });
    ...
    it('',
      async function () {
        packageJson.dependencies[packageName] = '<=8.5.2';
        const result = await resolver.resolve(packageJson);
        expect(result.dependencies[packageName]).to.be.equal('>=8.2.2 <=8.5.2');
      });
    ...
    it('',
      async function () {
        packageJson.dependencies[packageName] = '<=8.4.x';
        const result = await resolver.resolve(packageJson);
        expect(result.dependencies[packageName]).to.be.equal('>=8.2.2 <8.5.0');
      });
